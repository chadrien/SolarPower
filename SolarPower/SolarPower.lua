...
function SolarPower:PerformCycle(name, class, skipzero)
	...
	for test = cur+1, SOLARPOWER_MAXBLESSINGS+1 do
		if SolarPower:CanBuff(name, test) and (SolarPower:NeedsBuff(class, test) or shift) then
			cur = test
			do break end
		end
	end
...
function SolarPower:UpdateLayout()
	...
	if self.opt.layout == "Standard" then
		...
		if self:GetNumUnits() > 0 and not self.opt.disabled and PP_IsCleric and (self.opt.autobuff.autobutton or self.opt.hideClassButtons) then
			autob:Show()
			offset = offset - y
		else
			autob:Hide()
